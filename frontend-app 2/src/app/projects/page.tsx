...
import type { ProjectGroup, SearchParams, StackTag } from '@/types/content'
...
async function getTags(): Promise<StackTag[]> {
  const query = `*[_type == 'stackTags'] {
    _id, name, icon,
    'iconUrl': icon.asset->url
  }`

  const res = await sanityFetch<StackTag[]>({
    query,
...
async function getProjects(stack?: string): Promise<ProjectGroup[]> {
  if (!stack) return []
  ...
  const res = await sanityFetch<ProjectGroup[]>({
    query,
    params: { stack },
    tags: ['stackTags', 'projects'],
  })
